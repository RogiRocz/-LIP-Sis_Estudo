...
const configTab = tabsNavigation.value.find((tab) => tab.routeName === 'configuracoes');
...
<template>
  <v-menu location="bottom" transition="slide-y-transition">
    ...
    <v-list density="compact">
      ...
      <v-list-item :to="{ name: configTab.routeName }" :prepend-icon="configTab.iconName" class="mt-2">
        <v-list-item-title>{{ configTab.name }}</v-list-item-title>
      </v-list-item>

      <v-divider></v-divider>

      <v-list-item @click="logout" prepend-icon="logout" class="mt-2" base-color="error">
