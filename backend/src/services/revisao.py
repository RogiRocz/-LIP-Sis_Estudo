...
class RevisaoService:
    ...
    async def get_revisoes_for_user(self, user_id: int, page: int, size: int) -> Dict[str, Any]:
        if page < 1:
        ...
        revisoes, total = await self.repo.get_paginated_all_revisoes_by_user_id(user_id, skip, limit)
    ...
    async def get_cronograma(self, user_id: int, page: int, size: int) -> Dict[str, Any]:
        if page < 1:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="A página tem que ser maior ou igual a 1")
        if size < 1:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="O tamanho tem que ser maior ou igual a 1")
        ...
        revisoes, total = await self.repo.get_paginated_cronograma_by_user_id(user_id, skip, limit)
