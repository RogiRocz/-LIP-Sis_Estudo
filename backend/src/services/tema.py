...
class TemaService:
    ...
    async def create_tema_for_disciplina(
        self, disciplina_id: int, tema: TemaCreate, user: UserModel
    ) -> TemaModel:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Erro ao criar tema",
            )

    async def get_temas_by_disciplina(
        self, disciplina_id: int, user_id: int, page: int, size: int
    ) -> Dict[str, Any]:
        ...
        temas, total = await self.repo.get_paginated_temas_by_disciplina_id(
            disciplina_id, skip, limit
        )
    ...
    async def get_tema(self, tema_id: int, user_id: int) -> TemaModel:
        ...
        if not db_tema:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Tema não encontrado"
            )
        return db_tema

    async def update_tema(
        self, tema_id: int, tema_update: TemaUpdate, user_id: int
    ) -> TemaModel:
