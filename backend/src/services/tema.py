...
class TemaService:
    ...
    async def update_tema(self, tema_id: int, tema_update: TemaUpdate, user_id: int) -> TemaModel:
        db_tema = await self.get_tema(tema_id, user_id) # Reuse get_tema to ensure it exists and user has access
        return await self.repo.update_tema(db_tema, tema_update)

    async def delete_tema(self, tema_id: int, user_id: int):
        db_tema = await self.get_tema(tema_id, user_id) # Reuse get_tema for validation
