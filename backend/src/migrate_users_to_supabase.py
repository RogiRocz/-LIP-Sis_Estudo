        ...
        for user_id, email, nome in users_to_migrate:
            try:
                ...
                if email in supabase_users_map:
                    ...
                    print(
                        f"  ! Usuário já existe no Auth (ID: {supabase_id}). Vinculando..."
                    )
                else:
                    ...
                    try:
                        ...
                        print(
                            f"  ✗ Erro ao criar/localizar no Supabase: {create_error}"
                        )
                        error_count += 1
                        continue
                ...
                if supabase_id:
                    await conn.execute(
                        text(
                            'UPDATE "Usuario" SET supabase_id = :supabase_id WHERE "ID" = :user_id'
                        ),
